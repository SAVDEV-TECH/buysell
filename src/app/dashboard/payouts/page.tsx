  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-card rounded-2xl border border-border p-6 flex flex-col justify-between space-y-6 shadow-sm">
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-xs font-bold uppercase tracking-wider text-foreground flex items-center gap-2">
                <Building2 size={15} className="text-primary" /> Settlement Account
              </h3>
              <button
            ...
            {payoutSettings ? (
              <div className="p-4 rounded-xl bg-muted/40 border border-border space-y-2.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-bold text-foreground">{payoutSettings.bank_name}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Account Holder</span>
                  <span className="text-xs font-bold text-foreground">{payoutSettings.account_name}</span>
                </div>
                <div className="flex items-center justify-between">
                ...
                {payoutSettings.swift_code && (
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">SWIFT / BIC</span>
                    <span className="text-xs font-mono font-bold text-foreground">{payoutSettings.swift_code}</span>
                  </div>
                )}
              </div>
            ) : (
              <div className="p-6 text-center bg-muted/30 rounded-xl border border-dashed border-border space-y-3">
                <CreditCard size={24} className="text-muted-foreground mx-auto" />
                <p className="text-xs text-muted-foreground font-medium">No settlement bank account configured yet.</p>
                <button
                  onClick={() => setShowSettingsModal(true)}
                  className="px-4 py-2 bg-primary text-white text-xs font-bold rounded-xl shadow-sm hover:bg-primary/90 transition-all"
                >
                  Setup Bank Details
                </button>
              </div>
          ...
          <div className="p-3.5 rounded-xl bg-primary/10 border border-primary/20 text-xs text-muted-foreground space-y-1">
      ...
      <div className="bg-card rounded-2xl border border-border overflow-hidden space-y-4 shadow-sm">
        <div className="px-6 pt-5 flex justify-between items-center">
          <div>
            <h3 className="text-base font-bold text-foreground flex items-center gap-2">
              <History size={16} className="text-primary" /> Settlement Withdrawal Ledger
            </h3>
        ...
        {loading ? (
          <div className="p-16 flex flex-col items-center justify-center">
            ...
            <p className="text-xs text-muted-foreground font-bold uppercase tracking-wider">Loading ledger history…</p>
          </div>
        ) : payoutHistory.length === 0 ? (
          <div className="p-16 text-center space-y-2">
            <div className="w-10 h-10 bg-muted rounded-xl flex items-center justify-center mx-auto text-muted-foreground">
              <History size={20} />
            </div>
            <p className="text-xs text-muted-foreground font-medium">No withdrawal requests submitted yet.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-border bg-muted/40">
                  <th className="px-6 py-3.5 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Request Ref</th>
                  <th className="px-6 py-3.5 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Gross Amount</th>
                  <th className="px-6 py-3.5 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Wire Fee</th>
                  <th className="px-6 py-3.5 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Net Payout</th>
                  <th className="px-6 py-3.5 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Destination Bank</th>
                  <th className="px-6 py-3.5 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {payoutHistory.map((payout) => {
                  ...
                  return (
                    <tr key={payout.id} className="hover:bg-muted/40 transition-colors">
