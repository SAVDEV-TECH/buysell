...
export default function UserGuidedTourTooltip() {
  ...
  return (
    <div className="fixed top-20 right-4 sm:right-12 z-[100] max-w-sm bg-white dark:bg-slate-900 border border-primary/30 rounded-2xl shadow-2xl p-5 animate-in fade-in slide-in-from-top-4 duration-300">
      
      {/* Arrow pointing up towards top Navbar user profile badge */}
      <div className="absolute -top-2.5 right-12 w-5 h-5 bg-white dark:bg-slate-900 border-t border-l border-primary/30 rotate-45" />
        ...
        <div className="space-y-3">
          ...
          <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-700/60 flex items-center gap-2.5">
            ...
            <div className="text-xs">
              <span className="font-bold text-slate-900 dark:text-white">Your Dashboard:</span> Click your name/avatar in the top header anytime to access your orders, RFQs, and settings.
            </div>
          </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 pt-1">
            <Link
              href="/marketplace"
              onClick={handleDismiss}
              className="flex-1 h-8 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-xs font-bold hover:bg-slate-200 dark:hover:bg-slate-700 flex items-center justify-center transition-colors"
            >
              Browse Products
            </Link>
            <Link
