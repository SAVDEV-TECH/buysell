...
exports.cleanupOldLoginAttempts = exports.recordLoginSuccess = exports.clearLoginAttempts = exports.recordLoginFailure = exports.checkLoginRateLimit = void 0;
...
exports.checkLoginRateLimit = functions.https.onCall(async (data, context) => {
    ...
    const data_val = doc.data();
    const now = Date.now();
    // Check if account is locked
    if (data_val.locked && data_val.lockedUntil && now < data_val.lockedUntil) {
        throw new functions.https.HttpsError("permission-denied", `Account temporarily locked. Try again in ${Math.ceil((data_val.lockedUntil - now) / 1000 / 60)} minutes.`);
    }
    ...
    const attempts = (data_val.attempts || []);
    const recentAttempts = attempts.filter((ts) => now - ts < RATE_LIMIT_CONFIG.attemptWindowMs);
    if (recentAttempts.length >= RATE_LIMIT_CONFIG.maxAttempts) {
        ...
        throw new functions.https.HttpsError("permission-denied", "Too many login attempts. Account locked for 15 minutes.");
    }
    return {
        allowed: true,
 ...
 */
exports.recordLoginSuccess = functions.https.onCall(async (data, context) => {
 ...
 */
exports.cleanupOldLoginAttempts = functions.pubsub
